...
def macro_prf1(acc: "Accumulator") -> tuple:
...
def evaluate(llm_csv: Path, limit: int = None) -> dict:
    ...
    def report_scenario(label: str, acc: Accumulator, n_instances: int):
        ...
        R(f"    Micro F1        : {f_:.4f}")
        mp_, mr_, mf_ = macro_prf1(acc)
        R(f"    Macro Precision : {mp_:.4f}  (mean over prefix-type buckets)")
        R(f"    Macro Recall    : {mr_:.4f}  (mean over prefix-type buckets)")
        R(f"    Macro F1        : {mf_:.4f}  (mean over prefix-type buckets)")
