...
class Accumulator:
    def __init__(self):
        ...
        self.instance_metrics = []  # (precision, recall, f1) per aligned instance
    ...
    def add(self, match_result: dict):
        ...
        self.instance_metrics.append(prf1(t["tp"], t["pb"], t["rb"]))
    ...
    def merge(self, other: "Accumulator"):
        ...
        self.instance_metrics.extend(other.instance_metrics)


def macro_prf1_per_bucket(acc: "Accumulator") -> tuple:
    """Macro-averaged P/R/F1 across the prefix-type buckets (raw,
    no_evident_not, have_evident), each weighted equally regardless of how
    many literal pairs it contains.

    The paper does not define a macro-average for Objective 2 -- its only
    aggregate formula for this objective is the Micro one (Eq. 4.14-4.16:
    pool tp/pb/rb across every instance, divide once). It does, however,
    define "macro-averaged" for Objective 3 (Sec 4.6.1) as: compute the
    metric independently per group from that group's aggregated TP/FP/FN
    counts, then take the arithmetic mean across groups, so that every group
    counts equally no matter its size. This reuses that same recipe here,
    substituting Objective 2's three prefix-type buckets (which already
    partition every literal pair, mirroring Objective 3's G1-G4 groups) for
    Objective 3's four polarity groups.

    A bucket with no predicted and no expected literals (pb=0 and rb=0) has
...
def macro_prf1_per_instance(acc: "Accumulator") -> tuple:
    """Macro-averaged P/R/F1 across individual aligned instances (each
    review/claim), every instance weighted equally regardless of how many
    literal pairs it contains.

    This is the more common textbook definition of "macro-averaging" in NLP
    literature (equal weight per document/sample), distinct from the
    paper's own use of the term for Objective 3, which averages across
    combination groups (G1-G4) rather than individual instances. Each term
    is simply the mean of the per-instance Eq. 4.11-4.13 values already
    computed for every aligned instance."""
    if not acc.instance_metrics:
        return 0.0, 0.0, 0.0
    n = len(acc.instance_metrics)
    p_sum = sum(p for p, r, f in acc.instance_metrics)
    r_sum = sum(r for p, r, f in acc.instance_metrics)
    f_sum = sum(f for p, r, f in acc.instance_metrics)
...
def evaluate(llm_csv: Path, limit: int = None) -> dict:
    ...
    def report_scenario(label: str, acc: Accumulator, n_instances: int):
        ...
        R(f"    Micro F1        : {f_:.4f}")
        mp_, mr_, mf_ = macro_prf1_per_bucket(acc)
        R(f"    Macro Precision (per-bucket)   : {mp_:.4f}  (mean over prefix-type buckets)")
        R(f"    Macro Recall    (per-bucket)   : {mr_:.4f}  (mean over prefix-type buckets)")
        R(f"    Macro F1        (per-bucket)   : {mf_:.4f}  (mean over prefix-type buckets)")
        ip_, ir_, if_ = macro_prf1_per_instance(acc)
        R(f"    Macro Precision (per-instance) : {ip_:.4f}  (mean over aligned instances, n={len(acc.instance_metrics)})")
        R(f"    Macro Recall    (per-instance) : {ir_:.4f}  (mean over aligned instances, n={len(acc.instance_metrics)})")
        R(f"    Macro F1        (per-instance) : {if_:.4f}  (mean over aligned instances, n={len(acc.instance_metrics)})")
